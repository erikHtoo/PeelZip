...
def extract_aggressive(source, destination, decoder=None, progress=None,
    ...
    for number, block in enumerate(blocks, 1):
        ...
        command = [str(decoder), 'x', '-y', f'-o{destination}', str(source), *names]
        ...
        ntfs_reclaim.reclaim_range(source, block['offset'], block['packed'])
        reclaimed += block['packed']
        state['blocks'][key] = {'offset': block['offset'], 'packed': block['packed'], 'names': names}
